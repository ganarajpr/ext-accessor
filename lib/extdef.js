...
function checkIfPathReturnsAValue(path) {
    if(path.name === 'init' || path.name === 'right'){
        return true;
    }
    return false;
}
...
function isExpressionGlobal(exprList){
    var i = 0;
    var currentObject = startedRepl.context;
    var currentExpr;
...
function getListOfExternals(context,parentVariables){
    ...
    var filteredExterns = _.filter(context.uses,function(ext){
        return !_.contains(consolidatedVars,ext.raw[0]);
    });
    ...
    var joinedExterns = _.map(afterGlobalFilter,function(ext){
        return ext.raw.join(".");
    });
    ...
    context.externals = _.uniq(joinedExterns);
