...
function getExpressionForNode(path){
    ...
    expr.path = utils.getFullPath(path);
    return expr;
}
...
function createContextInfo(fnPath){
    ...
    currentContext.path = utils.getFullPath(fnPath);
...
function getSanitizedExpression(expr){
    ...
    if(utils.isFunction(expr)){
        currentExpr = utils.removeFunctionBrackets(expr);
    }
    else{
        currentExpr = expr;
    }
...
function getListOfExternals(context,parentVariables){
    ...
    var joinedExterns = _.filter(filteredExterns,function(ext){
...
exports.find = function(fnpath){
    ...
    infer.analyze(rootContext,fnpath);
